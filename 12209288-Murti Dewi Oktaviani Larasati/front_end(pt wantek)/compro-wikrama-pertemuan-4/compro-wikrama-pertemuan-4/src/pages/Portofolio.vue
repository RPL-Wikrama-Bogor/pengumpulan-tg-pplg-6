    ...
    export default{
      ...
      methods:{
        async filter(id){
            ...
            Get('http://127.0.0.1:8000/api/portfolio?category_id=' + id);
      ...
      async created() {
        const response = await Get('http://127.0.0.1:8000/api/portfolio');
        this.DataPortfolio = response.data.data;

        const responseCategories = await Get('http://127.0.0.1:8000/api/categories');
