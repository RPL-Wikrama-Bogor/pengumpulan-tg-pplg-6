...
use App\Exports\OrderExport; // Removed the duplicate import
use PDF;
...
class OrderController extends Controller
{
    public function export() {
        ...
        return Excel::download(new OrderExport, 'Orderan.xlsx');
    }
    ...
    public function store(Request $request)
    {
        ...
        //     // $getStock = Medicine::where('stock', $id)->first();
        //     $reduceStock = $medicine['stock'] - $count;

        // }
        foreach ($arrayDistinct as $id => $count) {
            $medicine = Medicine::where('id', $id)->first();
        
            if ($medicine && $medicine->stock >= $count) {
                $subPrice = $medicine['price'] * $count;
