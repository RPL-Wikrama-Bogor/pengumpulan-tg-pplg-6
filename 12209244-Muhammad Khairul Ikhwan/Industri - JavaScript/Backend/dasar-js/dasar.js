...
//Buatlah sebuah fungsi Javascript yang menggunakan map untuk mengembalikan daftar barang dengan harga diskon 10%
function hitungDiskon(daftarBarang) {
  
  const daftarDiskon = daftarBarang.map((barang) => {
    const hargaDiskon = barang.harga * 0.9; 
    return { ...barang, harga: hargaDiskon }; 
  });

  return daftarDiskon;
}

const daftarBarangAwal = [
  { nama: "Sepatu", harga: 100 },
  { nama: "Baju", harga: 50 },
  { nama: "Tas", harga: 75 },
];

const daftarBarangDenganDiskon = hitungDiskon(daftarBarangAwal);

console.log(daftarBarangDenganDiskon);
